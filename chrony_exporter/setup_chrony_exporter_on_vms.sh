...
# Function to copy and configure chrony_exporter on a remote VM
setup_chrony_exporter_on_vm() {
    local vm_ip=$1
    ...
    if ssh -o ConnectTimeout=5 "$vm_ip" 'netstat -an | grep LISTEN | grep ":9109"' &>/dev/null; then
        echo "Port 9109 is already listening on $vm_ip. Skipping setup."
        return
    fi
...
for vm_ip in "${vm_ips[@]}"; do
    echo "Setting up chrony_exporter on VM: $vm_ip"
    setup_chrony_exporter_on_vm "$vm_ip"
done
